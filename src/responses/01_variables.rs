fn main() {
  ...
  // **Posible solución al ejercicio práctico**
  const MY_AGE: u32 = 30; // Paso 1
  let mut my_name = String::from("Santiago"); // Paso 2
  my_name.push_str(" Viana"); // Modificar el nombre completo

  let my_tuple = (MY_AGE, 1.75, 'S'); // Paso 3
  let my_array = ["Rust", "is", "awesome"]; // Paso 4

  println!("\n--- Solución al ejercicio práctico ---");
  println!("Constante con mi edad: {}", MY_AGE);
  println!("Nombre completo: {}", my_name);
  println!("Tupla: {:?}", my_tuple);
  println!("Array: {:?}", my_array);

  let my_age = MY_AGE; // Shadowing (Paso 5)
  let my_age = my_age + 1;
  println!("Edad después de shadowing: {}", my_age);

  unsafe {
      static mut MY_GREETING: &str = "Hello, Rustacean!"; // Paso 6
      MY_GREETING = "Welcome to Rust!";
      println!("Saludo modificado: {}", MY_GREETING);
  }
}
