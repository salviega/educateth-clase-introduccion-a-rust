...
fn main() {
    ...
    println!("**Solución: Entrada y salida en Rust**");

    // **1. Escribir mensajes en la consola**
    println!("¡Hola! Este programa calculará cuántos días has vivido basado en tu edad.\n");

    // **2. Leer una cadena de texto**
    println!("¿En qué ciudad vives?");
    ...
    io::stdin()
        ...
        .expect("Error al leer la entrada.");
    let edad: i32 = edad_input.trim().parse().expect("Por favor, ingresa un número válido.");
    ...
    println!("\nResumen:");
    println!("Vives en: {}", ciudad.trim());
    println!("Tienes: {} años", edad);
    println!("Has vivido aproximadamente: {} días", dias_vividos);
